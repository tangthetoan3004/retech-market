...
export default function RoleCreatePage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await createRole({ title: title.trim(), description: description.trim() });
    ...
    <div className="p-6 bg-background text-foreground min-h-screen max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Định nghĩa một nhóm quyền hạn mới cho hệ thống</p>
        </div>
      </div>
          ...
          <div className="pt-6 border-t border-border flex gap-3">
            ...
            >
              {saving ? (
                ...
                <><Save className="w-4 h-4" /> Tạo Nhóm Quyền</>
              )}
            </Button>
            <Link to="/admin/roles">
